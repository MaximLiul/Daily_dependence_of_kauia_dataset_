...
def activity_periods_of_the_day(data_frame, column_for_analysis, date_column, quantity_column,
                                transaction_column, number_of_months_in_interval, product_or_user = 'user', list_of_excluded_hours = [],
    ...
    list_of_considered_months = np.append(list_of_considered_months, list_of_considered_months[len(list_of_considered_months) - 1] + 1)
           ...
           # we use min() function in order to prevent the situation when index will exceed an array's length
            df_selected.loc[(df_selected[date_column] >= list_of_considered_months[
                min_months_value_in_interval]) & (df_selected[date_column] <= list_of_considered_months[
                 min(min_months_value_in_interval +  number_of_months_in_interval - 1, len(list_of_considered_months) - 1)]), 'Time Interval'] = \
                        '{}-{}'.format(list_of_considered_months[min_months_value_in_interval],
                            list_of_considered_months[min(min_months_value_in_interval + number_of_months_in_interval, len(list_of_considered_months) - 1)])
            print(df_selected['Time Interval'].unique())
            min_months_value_in_interval+= number_of_months_in_interval
            print(min_months_value_in_interval)
            print(list_of_considered_months[len(list_of_considered_months) - 2])
   ...
   # creation of a df, where indexes are product_or_user and colums are time intervals
    ...
    if select_product_or_user == False:
        return df_grouped_by_column_for_analysis_time_interval.sort_values('Total', ascending=False).to_csv(
                            'activity_periods_months_{}_number_of_months_{}_select_{}.csv'.format(
                                product_or_user, number_of_months_in_interval, select_product_or_user))
    else:
...
activity_periods_of_the_day(df, 'Member ID', 'Transaction Date', 'Product Quantity',
                                'Transaction ID', 1, product_or_user = 'user', list_of_excluded_hours = [12],
                                select_product_or_user = False)