...
def activity_periods_of_the_day(data_frame, column_for_analysis, date_column, quantity_column,
                                transaction_column, number_of_intervals, product_or_user = 'user', list_of_excluded_hours = [],
    ...
    number_of_hours_in_the_interval = int(math.ceil(len(list_of_considered_hours)/number_of_intervals))
    ...
    for interval_counter in range(number_of_intervals):
        if interval_counter*number_of_hours_in_the_interval < list_of_considered_hours[len(list_of_considered_hours)-1]:
        ...
        # we use min() function in order to prevent the situation when index will exceed an array's length
            df_selected.loc[(df_selected[date_column] >= list_of_considered_hours[
                interval_counter * number_of_hours_in_the_interval]) & (df_selected[date_column] <= list_of_considered_hours[
                 min((interval_counter + 1) * number_of_hours_in_the_interval - 1, len(list_of_considered_hours) - 1)]), 'Time Interval'] = \
                        '{}-{}'.format(list_of_considered_hours[interval_counter * number_of_hours_in_the_interval],
                            list_of_considered_hours[min((interval_counter + 1) * number_of_hours_in_the_interval, len(list_of_considered_hours) - 1)])
       # if list_of_considered_hours[interval_counter * number_of_hours_in_the_interval] ==
   ...
   # creation of a df, where indexes are product_or_user and colums are time intervals
    ...
    if select_product_or_user == False:
        return df_grouped_by_column_for_analysis_time_interval.sort_values('Total', ascending=False).to_csv(
                            'activity_periods_of_the_day_{}_number_of_intervals_{}_select_{}.csv'.format(
...
activity_periods_of_the_day(df, 'Member ID', 'Transaction Date', 'Product Quantity',
                                'Transaction ID', 4, product_or_user = 'user', list_of_excluded_hours = [0,1,18,19,20,21,22,23],
                                select_product_or_user = False)